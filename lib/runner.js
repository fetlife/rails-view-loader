 ...
 */
const run = (loader, config, map, callback) => {
...
const retryRun = (loader, config, map, callback) => {
  const operation = retry.operation({ retries: 3, minTimeout: 0, maxTimeout: 10 })
  operation.attempt((currentAttempt) => {
    run(loader, config, map, (error, output, map) => {
...
module.exports = (loader, config, map, callback) => {
  retryRun(loader, config, map, callback)
}
